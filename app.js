...
app.set('port', process.env.PORT || 9000); //앱의 포트를 설정 -> 환경 변수에 포트가 지정되어 있지 않으면 3000번 포트를 사용
    //app.set(키, 값)에 데이터를 저장, 데이터를app.get(키)로 가져올 수 있음.
...
app.use(morgan('dev')); //morgan('dev') -> 요청과 응답에 대한 정보를 콘솔에 기록
    ...
    app.use(express.urlencoded({ extended: false }));
        //주소 형식 데이터 해석 -> req.body 객체이 저장
        //{ extend: false } -> 노드의 qureystring 모듈을 사용해 쿼리스트링 해석

    //요청에 등봉된 쿠키를 해석 -> req.cookies 객체에 저장
    ...
    app.use(session({
        ...
        //saveUnitialized -> 세션에 저장할 내역이 없더라도 처음부터 세션을 생성할지 설정
        //secret -> 안전하게 쿠키를 전송하려면 쿠키에 서명을 추가해야 함. 쿠키에 서명할 떄 secter 값이 필요
        //cookie -> 세션 쿠키에 대한 설정
    ...
    const upload = multer({
        ...
        //업로드될 파일의 최대 크기 제한 -> 5MB(5 * 1024 * 1025byte)로 설정
    ...
    //'/upload' 경로로 POST요청이 오면, upload 미들웨어 사용 -> 이미지 파일을 업로드
    ...
    app.get('/', (req, res, next) => {
        ...
        //app.het(주소, 라우터) -> 주소에 대한GET요청이 올떄 어떤 동작을 할지 적는 부분
    ...
    //에러처리 미들웨어는 특별한 경우가 아니면 아래에 위치함.
